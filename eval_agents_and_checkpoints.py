...
def main():
    ...
    checkpoint_path=curr_dir+'/cluster_daten/one_environment_2000/oneenvironmentima_'+num_inst+'_'+num_inst+'_'+'2000_sims'+'/checkpoint-'
    ...
    for loading_instance in train_instances:#train_instances:
        every_action_time=[]
        train_rewards=[]
        every_action=[]
    #    for sim in num_sims:
        ...
        for iteration in range(1,num_checkpoints+1):
            ...
            # write results
            results[get_instance_name(loading_instance),iteration]={'train_reward':reward,'every_action_time':time_list,'every_action':action_list,'checkpoint nr':iteration}
            #results[sim]={'train_reward':train_rewards,'every_action_time':every_action_time,'every_action':every_action}

            df=pd.DataFrame.from_dict(results)
            df.to_csv(f"eval_size{num_inst}_inst_4and5_{num_checkpoints}_2sims_1_5puct_2000sims.csv")   
