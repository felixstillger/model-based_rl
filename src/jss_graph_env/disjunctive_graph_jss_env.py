...
class DisjunctiveGraphJssEnv(gym.Env):
    ...
    def __init__(self,
        ...
        #reward mode, switch for not sparse rewards from makespan to utilisation

        self.reward_mode = reward_mode if reward_mode is not None else "makespan"
    ...
    def step(self, action: int) -> (np.ndarray, float, bool, dict):
        ...
        # change to total time in parallel and not overlapping jobs at start time    
            ...
            if task_id != None:
                curr_job_node = self.G.nodes[task_id]
                curr_start=curr_job_node['start_time']
                if not isinstance(curr_start,tuple):
                    # print(f"curr start is {curr_start} with type {type(curr_start)}")
                    curr_finish=curr_job_node['finish_time']
                    if curr_finish is not tuple:

                        curr_job_length=curr_finish-curr_start
                    
                        #print(f"current finish is {curr_finish} with type {type(curr_finish)}")
                    # print(f"debug: {curr_time}")

                    # active machines should count the current active machines. Set to -1 because own machine is counted too
                    active_machines=-1
                    # time norm should take a norm for parallel scheduled jobs.
                    time_norm=0
                    
                    # overlappint time is counted for each task within the production time of current added job
                    overlapping_time=0

                    for _ in range(self.total_tasks):
                        tmp_start=0
                        tmp_finish=0
                        if self.G.nodes[_]['finish_time'] is not None and curr_start <= self.G.nodes[_]['finish_time']:
                            if self.G.nodes[_]['start_time'] is not None and self.G.nodes[_]['start_time']<= curr_finish:

                                # check if the task starts before the current task, only parallel time counts.
                                tmp_start=self.G.nodes[_]['start_time'] if self.G.nodes[_]['start_time'] >= curr_start else curr_start
                                
                                tmp_finish=curr_finish if self.G.nodes[_]['finish_time'] >= curr_finish else self.G.nodes[_]['finish_time']
                                if (tmp_finish-tmp_start) < 0:
                                    raise ValueError("parallel time is lower than zero")
                                overlapping_time += (tmp_finish-tmp_start)
                                active_machines =active_machines + 1

                    # print(f"debug: detected {active_machines} active machines")
                    #old reward with machines
                    #reward = -(self.n_machines - active_machines)
                    # new reward with overlapping time, time norm should be at least 1/n_machines and maximum of 1
                    # used round method, because machine error makes time norm in the next check less 1/n_machines if it is the only scheduled task

                    time_norm=round((overlapping_time/self.n_machines)/(curr_finish-curr_start),2)
                    if time_norm < 0.95 * 1/self.n_machines or time_norm > 1:
                        raise ValueError(f"time_norm is equal: {time_norm} and this is not plausible")

                    reward=time_norm
                    #print(f"reward is: {reward}")
                else:
                    # print(curr_time)
                    # possible if no valid action is chosen
                    # bestrafung irgendwie 
                    # reward = - self.n_machines
                    reward = -1


            else:
                raise RuntimeError(f"utilisation only implemented for task mode")
        ...
        if done:
            ...
            if self.reward_mode=="makespan":
                ...
                if False:
                    makespan = nx.dag_longest_path_length(self.G)
                    reward = reward - (makespan / self.scaling_divisor) if self.scale_reward else reward - makespan
        ...
        if action_mode == 'task':
            ...
            if True not in mask:
                ...
                if not self.env_transform == 'mask':
                    print(min([len(route) for m_id, route in self.machine_routes.items()]))
                    print(done)
