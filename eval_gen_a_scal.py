...
def get_model(iteration):
    return str(ids[iteration%3])
def get_size(iteration):
    size_inter=[6,8,10,15]
    return str(size_inter[int(floor(iteration/3))])
all_inst=[15,10,8,6]
def main():
    for num_inst_t in all_inst:
        num_inst_t=str(num_inst_t)
        ...
        for i in range(20,40):
            test_instances.append(curr_dir+'/resources/jsp_instances/ima/'+num_inst_t+'x'+num_inst_t+'x'+num_inst_t+'/'+num_inst_t+'x'+num_inst_t+'_'+str(i)+'_inst.json')
        ...
        #    for sim in num_sims:
            ...
            for iteration in range(0,len(checkpoints_p)):
                ...
                for c_nr,checkpoint_path in enumerate(checkpoint_list):
                    ...
                    # write results
                    results[get_instance_name(loading_instance)+"_"+get_model(iteration)+"_s"+get_size(iteration)]={'train_reward':reward,'every_action_time':time_list,'every_action':action_list,'checkpoint nr':c_nr}
                    #results[sim]={'train_reward':train_rewards,'every_action_time':every_action_time,'every_action':every_action}

                df=pd.DataFrame.from_dict(results)
                df.to_csv(f"gen_scale_tests_size{num_inst_t}_inst_20_50_100sims_1_5puct.csv")   
