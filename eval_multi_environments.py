...
num_inst=str(3)
out_episodes=20
checkpoint_dir='/Users/felix/sciebo/masterarbeit/progra/model-based_rl/training_checkpoints/ima_3_3_no_act_10inner/'
    ...
    for outer in range(out_episodes):
        ...
        for iteration in range(1,num_checkpoints+1):
            ...
            if os.path.exists(checkpoint_path):
                ...
                # write results
                results[(checkpoint_path)]={'train_reward':reward,'every_action_time':time_list,'every_action':action_list,'outer':outer,'iteration':iteration}
                df=pd.DataFrame.from_dict(results)
                df.to_csv(f"eval_{num_inst}_multi_inst_{num_checkpoints}_check.csv")   
            else:
