...
class jssp_klagenfurt_obs_wrapper(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        #print(obs)
        return {"obs": np.ravel(obs['real_obs']).astype(np.float64), "action_mask": obs['action_mask']}
    ...
    def set_state(self, state):
        ...
        return {"obs":np.ravel(self.env.state).astype(np.float64),"action_mask":self.env.legal_actions}
